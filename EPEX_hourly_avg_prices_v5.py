...
# Create subplots with specific types for the table
fig = make_subplots(
    rows=5, cols=1, 
    subplot_titles=('Day-Ahead Electricity Price over time', 'Average Price per Hour for various years', 'Average Price per Hour by Month for year 2024','DA Price Distribution', 'Annual Statistics Table'),
    specs=[[{"type": "xy"}],
            [{"type": "xy"}],
            [{"type": "xy"}],
            [{"type": "xy"}],
...
# Add traces for selected months to the third subplot
for month in [1, 4, 12]:
    if month in monthly_hourly_avg_prices.index:
...
# Add the fourth plot (DA Price Distribution - Histogram)
for year in years:
    year_data = df[df['year'] == year]['DA_price']
    fig.add_trace(go.Histogram(x=year_data, name=f'Year {year}', opacity=0.7), row=4, col=1)

# Calculate statistics for each year
yearly_stats = []
for year in years:
    year_data = df[df['year'] == year]['DA_price']
    stats = year_data.describe().round(1)
    yearly_stats.append({
        'Year': year,
        'Count': stats['count'],
        'Mean': stats['mean'],
...
# Create statistics table and transpose it
stats_df = pd.DataFrame(yearly_stats)
stats_df_transposed = stats_df.set_index('Year').T
print("Annual Statistics (Transposed):")
print(stats_df_transposed)

# Add the fifth plot (Statistics Table) - transposed
fig.add_trace(
    go.Table(
        header=dict(
            values=['Statistic'] + list(stats_df_transposed.columns),
        ...
        cells=dict(
            values=[stats_df_transposed.index] + [stats_df_transposed[col] for col in stats_df_transposed.columns],
            fill_color='lavender',
            align='left',
            font=dict(size=11),
            height=30
        ),
        columnwidth=[200] + [150] * len(stats_df_transposed.columns)
    ),
    row=5, col=1
)
...
fig.update_layout(
    height=1600,  # Increased height for 5 subplots including taller table
    showlegend=True,
    title_text="EPEX Day-Ahead Electricity Price Analysis"
)

# Save the main subplot figure as HTML
fig.write_html('EPEX_Prices_Analysis_v5.html', auto_open=True)
